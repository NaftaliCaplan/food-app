...
`outfitCandidates.ts` (the enumerator/scorer core), `outfitAesthetics.ts` (the scoring function), `outfitRecommendation.ts` (templated tip), `styleTags.ts` (shared style-tag normalization, used by both AI tagging and manual edits), `parseLLMResponse.ts` (legacy food-checker's markdown-fence-fallback parser).
